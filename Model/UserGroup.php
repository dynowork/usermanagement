...
class UserGroup extends UsermangmtAppModel {
	 ...
	 * @var array
	 */
	var $hasMany = array('Usermangmt.UserGroupPermission');
	/**
	...
	public function getPermissions($userGroupID) {
		...
		$actions = Cache::read($cacheKey, 'Usermangmt');
		if ($actions === false) {
			...
			Cache::write($cacheKey, $actions, 'Usermangmt');
		}
